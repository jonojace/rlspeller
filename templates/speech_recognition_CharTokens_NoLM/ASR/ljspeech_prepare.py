...
from templates.speech_recognition_CharTokens_NoLM.ASR.text_normalisation import cleaners
import tarfile
...
def prepare_ljspeech(
        ...
        text_cleaners,
        dump_feats: bool, # set to True if want to dump all acoustic features for corpus,
        # will create different annotation json files that collectively include ALL of the utterances
        # therefore ignoring utterance ids that are in uttids_to_excl
):
    """
    ...
    if skip(save_json_train, save_json_valid, save_json_test) and not dump_feats: # dont exit function early if dumping feats
        logger.info("Corpus preparation completed in previous run, skipping.")
        return
    ...
    # rename manifest file if dumping feats
    if dump_feats:
        dump_feats_append_str = "FOR_DUMPING_FEATS"
        save_json_train = append_str_to_filename(save_json_train, dump_feats_append_str)
        save_json_valid = append_str_to_filename(save_json_valid, dump_feats_append_str)
        save_json_test = append_str_to_filename(save_json_test, dump_feats_append_str)

        print("dump feats is true!!!")
    ...
    wavs_folder = os.path.join(ljspeech_folder, 'wavs')
    wav_list = get_all_files(wavs_folder, match_and=extension)
    n = len(wav_list)
    assert n == LJSPEECH_EXPECTED_N, f"{n=}, {LJSPEECH_EXPECTED_N=}, {wavs_folder=}"
    ...
    if dump_feats:
        # include all utterances in corpus, do not perform filtering
        valid_n = math.floor(valid_percent * n)
        test_n = math.floor(test_percent * n)
        train_n = n - (valid_n + test_n)
        assert train_n + valid_n + test_n == n, f"{train_n=} {valid_n=} {test_n=} {train_n+valid_n+test_n=} {n=}"
    else:
    ...
    if dump_feats:
        assert len(wav_list_train) + len(wav_list_valid) + len(wav_list_test) == n
    else:
